# Analyse PBDB data
 ...
 setwd("c:/daten/R/Lehre")
  ...
  pbd <- read.table(file="C:/Daten/PBDB/Occs0910.csv", header=TRUE, sep=";") 

   # Omit occurrences that cannot be assigned to a stage
    pbd <- subset(pbd, is.na(slc)==F)


 # Create and export an FAD-LAD matrix
  # source("FAD_LAD.R")

  # Load time scale
  # stage definitions (77)
    stages <- read.table(file="C:/Daten/PBDB/Stages77.csv", header=TRUE, sep=";")
     age <- stages$MEAN_AGE # mean age from PARED
     bin.n <- stages$STAGE
     tops <- stages$HIGH.PARED
      s <- 77   
...
ps <- tht/(tht+ptm) # Sampling probability = Three timer sampling stat 
    ps. <- c(ps[2:77],NA) # Sampling probability in subsequent bin
    tt. <- c(tt[2:77], NA) # Two timers in subsequent bin
...
# Plot 
  ...
  plot(age, see.t, type="l", xlim=c(550, 0), col="red")
   points(age, blind.t, type="l")